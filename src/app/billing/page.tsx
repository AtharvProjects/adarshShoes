...
function BillingCounter({ counterIndex, editId, onClearEdit }: { counterIndex: number, editId: string | null, onClearEdit: () => void }) {
  ...
  /* Fetch helpers — always bypass cache to prevent stale data */
  ...
  const handleSave = async (printAndClear = false) => {
    ...
    try {
      ...
      // Immediately re-fetch products so stock quantities are up-to-date
      fetchProducts();
      // Also re-fetch customers since walk-in customers may have been auto-created
      fetchCustomers();
        ...
        try {
          ...
          try {
            ...
            if (printArea && iframeDoc) {
              ...
              clone.style.overflow = 'visible'; // Prevent clipping
              clone.style.zIndex = '-9999';
              clone.style.pointerEvents = 'none';
              clone.style.backgroundColor = 'white';
              
              // Extract actual CSS rules from iframe's loaded stylesheets
              // instead of cloning <link> elements (which may not load in time)
              const styleContainer = document.createElement('div');
              styleContainer.id = '__pdf-styles__';
              
              try {
                const iframeSheets = Array.from(iframeDoc.styleSheets);
                for (const sheet of iframeSheets) {
                  try {
                    ...
                    // Cross-origin stylesheet — fall back to cloning the element
                    if (sheet.ownerNode) {
                      styleContainer.appendChild(sheet.ownerNode.cloneNode(true));
                    }
                  }
                }
              } catch (sheetErr) {
                // Fallback: clone all style/link elements if styleSheets API fails
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <Card className="border-2 border-primary/20 shadow-sm relative z-40 print:hidden">
      ...
      {/* Print Modal */}
      <Dialog open={printModalOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            ...
            <DialogTitle className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="outline" className="text-green-600 border-green-200 hover:bg-green-50" onClick={() => {
                   // Optional: implement WhatsApp sharing here or rely on the auto-send
                   toast.info("WhatsApp receipt was already sent automatically!");
                }}>
                  WhatsApp
                </Button>
              </div>
          ...
          <div className={settings.invoice_theme === 'thermal' ? "bg-white p-6 mx-auto w-[300px]" : "bg-white p-6 border border-gray-300 mt-4 text-black w-full"}>
              {savedInvoice && (
                <InvoiceTemplate 
