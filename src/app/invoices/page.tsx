...
export default function InvoicesPage() {
  ...
  /*
   * Offscreen capture: clone the invoice content into an unconstrained container
   * appended to document.body (outside any dialog/modal), then capture it at full
   * A4 width. This avoids viewport and dialog max-width constraints that cause
   * html-to-image to produce cropped output.
   */
  const generatePdfBase64Offscreen = async (): Promise<{ base64: string; fileName: string }> => {
    const node = billRef.current;
    if (!node) throw new Error('Invoice view is not fully rendered yet.');

    const A4_W_PX = 794; // 210mm at 96 DPI

    // 1. Deep-clone the invoice content
    const clone = node.cloneNode(true) as HTMLElement;

    // 2. Create an offscreen container attached directly to <body>
    const container = document.createElement('div');
    container.style.position = 'absolute';
    container.style.left = '-9999px';
    container.style.top = '0';
    container.style.width = `${A4_W_PX}px`;
    container.style.minWidth = `${A4_W_PX}px`;
    container.style.overflow = 'visible';
    container.style.backgroundColor = '#ffffff';
    container.style.zIndex = '-9999';
    container.style.pointerEvents = 'none';

    // 3. Style the clone for exact A4 width, natural height
    ...
    clone.style.padding = node.style.padding || '24px';
    ...
    // 4. Wait for full browser reflow
    ...
    try {
      ...
      const imgDataUrl = await lib.toPng(clone, {
      ...
      const pdfHeight = Math.max((actualHeight * pdfWidth) / A4_W_PX, 100);
  ...
  const captureDynamicHeight = async (fmt: 'png'|'jpeg' = 'png', quality = 1) => {
    ...
    const ancestors: Array<{ el: HTMLElement; mh: string; mw: string; ov: string; ox: string; oy: string }> = [];
    ...
    while (anc && anc !== document.body) {
      ...
      if (cs.overflow !== 'visible' || cs.overflowY !== 'visible' || cs.overflowX !== 'visible' ||
        ...
        ancestors.push({ 
          ...
          mw: anc.style.maxWidth, 
          ov: anc.style.overflow, 
          ox: anc.style.overflowX, 
          oy: anc.style.overflowY 
        });
    ...
    const restore = () => {
      ...
      for (const { el, mh, mw, ov, ox, oy } of ancestors) {
  ...
  const shareOnWhatsApp = async (inv: Invoice | null) => {
    ...
    try {
      ...
      let fileName = null;
      try {
        // Generate the PDF using an offscreen clone outside the dialog
        // to avoid dialog max-width constraints cropping the output
        const result = await generatePdfBase64Offscreen();
