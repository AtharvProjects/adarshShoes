...
if (process.env.NEXT_PHASE === 'phase-production-build') {
  ...
  try {
    ...
    console.error('[DB] Failed to open database, possibly corrupted. Creating a backup and starting fresh...', err);
    try {
      fs.renameSync(DB_PATH, DB_PATH + '.corrupted.' + Date.now());
      db = new Database(DB_PATH);
    } catch (renameErr) {
      console.error('[DB] CRITICAL ERROR: Could not recover from database corruption.', renameErr);
      // Create an in-memory DB as absolute fallback to prevent application crash
